...
const handlers = {
  ...
  'GET /api/friends': (req, res, user) => {
    ...
    const friends = db
      ...
      .all(user.id);
    const momentCounts = db
      .prepare(
        `SELECT owner_id, COUNT(*) as n FROM moments
         WHERE owner_id IN (SELECT friend_id FROM friendships WHERE user_id = ?)
         GROUP BY owner_id`
      )
      .all(user.id);
    const countByOwner = Object.fromEntries(momentCounts.map((r) => [r.owner_id, r.n]));

    const withOccasion = friends.map((friend) => ({
      ...publicFriend(friend),
      occasion: nextOccasion(friend),
      momentCount: countByOwner[friend.id] || 0,
    }));
    withOccasion.sort((a, b) => {
      const da = a.occasion ? a.occasion.days : Infinity;
      const db_ = b.occasion ? b.occasion.days : Infinity;
    ...
    return {
      status: 200,
      body: { friends: withOccasion, inviteCode: user.invite_code },
    };
  },
  ...
  'GET /api/friends/:id/moments': (req, res, user, body, cookies, params) => {
    ...
    return {
      status: 200,
      body: { friend: { ...publicFriend(friend), occasion: nextOccasion(friend) }, moments },
    };
  },
};
